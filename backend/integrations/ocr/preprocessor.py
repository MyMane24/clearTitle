...
def preprocess_pdf(src_pdf: Path, out_pdf: Path) -> Path:
    ...
    for page_num in range(len(doc)):
        page = doc[page_num]
        pix = page.get_pixmap(matrix=RENDER_MATRIX, colorspace=fitz.csRGB)  # type: ignore[attr-defined]
    ...
    for enhanced in enhanced_pages:
        ...
        new_page = result_doc.new_page(width=pil_img.width, height=pil_img.height)  # type: ignore[attr-defined]
        new_page.insert_image(new_page.rect, stream=img_bytes)  # type: ignore[attr-defined]
